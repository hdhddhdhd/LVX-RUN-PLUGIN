# lvx-base (基础包 / base) — 官方插件调用 / Official Plugin Call

## 中文

**包信息**：lvx-base 1.0.0，15 个文件（`lvx ls plugins -a` 可查）。随 `lvx init`
自动安装，是导入/执行/分词/采样/训练等的基础能力。
...
**Package**: lvx-base 1.0.0, 15 files (see `lvx ls plugins -a`). Installed
automatically by `lvx init`; it is the base capability set.
